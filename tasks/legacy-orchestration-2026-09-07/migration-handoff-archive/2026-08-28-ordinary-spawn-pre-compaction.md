...
- Go write authority is bounded to `cmd/crystal-server/archer_summons.go`,
  `hell_lord.go`, `world.go`, the ordinary-spawn death hook in
  `ordinary_pets.go`, new `monster_ordinary_spawn_test.go`, the bounded
  `safe_zone_healing_direct_test.go` ledger entry and one authenticated
  transcript.
