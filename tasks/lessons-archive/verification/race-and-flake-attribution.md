...
- Teleport Actions recurrence: the first fresh `go test -race -count=1 ./...`
  failed only in `TestSessionKirinIceThrustTranscript`, with the established
  test-side writes to `world.monsterAIRoll` and callback state racing the
  session ticker in `monsterAIRollLocked`; no stack entered Teleport files.
  Exact isolated race count-3 passed, as did the unchanged Teleport focused
  race count-3 and a second fresh unexcluded full-race run. The failed first
  run remains recorded and was not described as a pass.
...
- Symptom: the first current-candidate `go test -race ./... -count=1 -timeout=60m` exited 1 only in `TestLoverRecallNetworkQueuedTransitionsAreNotOverwritten`; the detector showed the world ticker writing Regen/Pot/Heal/Vamp fields in `tickPlayerNaturalRegenerationLocked` while `main.go` copied the captured live `*worldPlayer` for relationship delivery without `world.mu`.
- Root cause: `RelationshipTransition` captured the mutable player pointer during bootstrap and later dereferenced the complete struct outside the world lock. The new periodic writer made the pre-existing unsafe callback boundary deterministic.
- Prevention: asynchronous session callbacks must reacquire a lock-owned runtime snapshot at invocation time; never copy or traverse a captured live entity after bootstrap releases the lock.
- Verification: the callback now calls `playerRuntimeSnapshot(worldObjectID)` before delivery. Touched compile and `go test -race ./cmd/crystal-server -run '^TestLoverRecallNetworkQueuedTransitionsAreNotOverwritten$' -count=10 -timeout=20m` exited 0; a fresh full race remains required and the original exit-1 evidence must remain in the handoff.
- Terminal follow-up: review proved that the first snapshot fix still shallow-aliased Buffs/Flags/enemy-guild state. The runtime and transition visibility snapshots now detach every mutable collection consumed by ObjectPlayer serialization; the new detachment regression plus relationship/regen focused race-count-5 exits 0.
- Full-race attribution: after the terminal fixes, two fresh unexcluded `go test -race ./... -count=1 -timeout=60m` runs each exited 1 only at the pre-existing `TestSessionOmaMageRangeSlowFrozenTranscript` `[2 1]`/`[1]` RNG fixture, with no race-detector report and no Regen/relationship stack. Exact isolated race `-count=10` exited 0, and the explicitly excluded remainder race exited 0. Neither excluded nor failed run is a fresh full-race pass; the leaf records the due gate and unrelated attribution rather than changing OmaMage scope.
- Final current fingerprint: after the follow-up TownRevive deep-snapshot fix and its focused race-count-5 gate, a new unexcluded `go test -race ./... -count=1 -timeout=60m` exited 0 (server 86.418s). This final pass supersedes neither prior failure evidence nor the OmaMage flake attribution; it is the required fresh unexcluded acceptance run for the completed Regen candidate.
