...
### 2026-08-22 — AI=71 Behemoth must preserve branch-local timing, constructor RNG, and cell semantics

- Symptom: Behemoth review found several easy-to-miss divergences: Legacy close Bleeding lasts 5 seconds (not the 15-second Dazed duration), `ObjectRangeAttack` leaves `TargetID` at its default zero, `CompleteAttack` pushes the first Player in the front Cell before checking that Player's attack gate, and each spawned Hugger/PoisonHugger constructor consumes CoolEye/direction/initial-timer random draws before child target selection.
- Root cause: The implementation grouped similar poison constants, reused target-bearing range payloads, selected the first attackable Player instead of reproducing the Cell loop's break behavior, and materialized child monsters without the Legacy constructor call chain.
- Prevention: Keep AI=71's close Bleeding `(chance=15,duration=5,tick=1s)` separate from push Dazed `(chance=3,duration=15,tick=1s)` and impact Paralysis `(chance=15,duration=5,tick=1s)`; encode the Legacy default range payload; preserve Cell insertion order and the first-Player break; account for constructor RNG before selecting the captured child target and initialize the child lifetime from construction time.
- Verification: Go AI=71 production-entry tests cover close/range payloads, delayed ACAgility and paralysis, configured slave registration/capture/two-second gate, parent cleanup, and repeated/race runs. The batch's latest ordinary full-suite run reports only the pre-existing `TestSessionOmaMageRangeSlowFrozenTranscript` random-boundary failure; its latest full race run reports only the pre-existing DarkBody equipment-statistics race and GuildBuff shared-session-fixture race. None enters AI=71. Both repositories retain zero `.cs` changes.

### 2026-08-22 — AI=72 FinialTurtle must preserve inherited spawn, movement, and poison boundaries

- Symptom: FinialTurtle is a thin `MonsterObject` override, so a narrow attack-only port can miss the random constructor direction, the `Spawned` two-second action gate, movement during attack cooldown, or the fact that `CompleteAttack`/`CompleteRangeAttack` still resolve after the attacker dies.
- Root cause: Treating the subclass's close/range split as a standalone AI and reusing the generic delayed-action dead gate; also setting poison `TickAt` to impact+tick would postpone the Legacy `ApplyPoison` state transition, whose default `TickTime=0` is processed on the same target tick.
- Prevention: Trace the base `ProcessAI`/`FindTarget`/`ProcessTarget` chain and the subclass's exact overrides. Keep adjacent non-same-cell DC `ObjectAttack` at 300ms and same-cell/remote MC `ObjectRangeAttack` at `MaxDistance*50+500ms`; revalidate target/map but not attack distance or attacker liveness at impact. Preserve independent Slow `(8,15)` and Frozen `(15,5)` `PoisonTarget` rolls and initialize newly applied poison with the Legacy zero tick boundary.
- Verification: Go AI=72 production-entry tests cover common population, two-second spawn gate, six-cell admission, close/range payloads, Player/owned-Monster/Hero projections, attacker-dead queued hits, cross-map invalidation, dual poison state, and an authenticated `net.Pipe` transcript including cooldown `ObjectWalk`; focused normal `-count=10`, race `-count=3`, package/full normal tests, vet, and build passed. Full race remains blocked only by the pre-existing `TestGuildBuffSessionNewbieLoginReplacesStalePersistedBuff` equipment-stat/shared-fixture race, outside AI=72.

### 2026-08-22 — AI=73 TurtleKing Type=1 range actions share CompleteRangeAttack AOE

- Symptom: 初版 Go AI=73 把近距离 `ObjectRangeAttack(Type=1)` 当作单目标 MAC 命中，漏掉了 `CompleteRangeAttack` 的当前 `FindAllTargets(AttackRange, CurrentLocation)` 扫描以及 TurtleKing effect/Slow/Paralysis 副作用。
- Root cause: 按攻击入口的 close/range 分支命名拆分了 delayed action，而 Legacy 的 close case 4 和远程 Type=1 都排入同一个 `DelayedType.RangeDamage`，由同一个 `CompleteRangeAttack` resolver 处理。
- Prevention: 先按 delayed-action 类型追踪 admission、impact resolver 和所有副作用；AI=73 的所有 `ObjectRangeAttack(Type=1)` 均标记 AOE，impact 时先校验原始 target，再按当前 Cell insertion order 扫描七格目标；effect 使用原始 `Target.ObjectID`，每个成功目标独立取 SC/poison RNG，并在首个 miss 后停止。
- Verification: Go `TestTurtleKingCloseRangeTypeOneUsesRangeAreaResolver`、远程 area/fan-out、target/map revalidation 和 authenticated `net.Pipe` transcript 覆盖该边界；定向普通 `-count=10`、race `-count=3`、整包/全仓普通、vet、build 通过。全仓 race 仍仅失败于既有 `TestGuildBuffSessionNewbieLoginReplacesStalePersistedBuff` equipment-stat/shared-fixture race，未进入 AI=73。

### 2026-08-22 — AI=74 LightTurtle must preserve base Attack and CompleteAttack boundaries

- Symptom: LightTurtle looks like a small two-branch override, so porting only the `Type=1` packet can lose the inherited `base.Attack()` path, its ACAgility damage, or the second SC draw hidden inside `PoisonTarget`.
- Root cause: The Legacy override chooses `Next(3) > 0` for the ordinary base attack and otherwise emits `ObjectAttack(Type=1)`; both branches schedule a 300ms `DelayedType.Damage` action. `CompleteAttack` rechecks only target attackability, same-map identity, and node presence, then calls `PoisonTarget(target, 4, GetAttackPower(MinSC, MaxSC), Green, 1000)`, whose body samples a separate SC value before resistance/chance gates.
- Prevention: Keep both attack branches on the same delayed ACAgility resolver, preserve `Next(3)` and fixed DC/SC random-call order, and revalidate target/map at impact without adding a distance or attacker-dead gate. Preserve the inherited adjacent non-same-cell search/movement lifecycle and constructor-random direction.
- Verification: Go AI=74 production-entry tests cover common population, both attack payloads, target-kind projection, random bounds, poison duration/value and resistance/chance sequence, moved-target versus cross-map invalidation, attacker-dead queued impact, and authenticated `net.Pipe` packet order. Focused normal/race gates and full ordinary `go test ./...` pass; full race remains blocked by the pre-existing GuildBuff equipment-stat/shared-session-fixture race outside AI=74.

### 2026-08-22 — AI=75 WitchDoctor must preserve base constructor RNG and branch-local timers

- Symptom: WitchDoctor is a small `ProcessTarget`/`Attack` override, so an attack-only port can silently retain the spawn-record direction instead of the Legacy base constructor's random direction, or apply ActionTime/AttackTime to the failed-teleport branch where Legacy applies neither. The first in-range pass can also be mistaken for an immediate attack instead of setting FearTime and retreating.
- Root cause: tracing only `WitchDoctor.cs` and not the `MonsterObject` constructor, `ProcessTarget` ordering, and `TeleportRandom` return path; branch-local side effects were collapsed into one generic attack cooldown.
- Prevention: derive the checklist from the full call chain: constructor `Random.Next(8)`, `FearTime` gate before Shock/retreat, 1-in-5 teleport with ten target-relative attempts, heal/range branch timers only after their packet, and `RangeDamage` revalidation with `MACAgility`. Keep the exact `Next(5)`, `Next(3)`, `Next(13)` sequence and preserve Cell insertion order plus Player/owned-Monster/Hero projections.
- Verification: Go AI=75 production-entry tests cover common population, constructor direction, cell-order search, Fear retreat, Shock clearing, self-heal, ranged impact, all target kinds, safe-zone invalidation, and exhausted teleport attempts; authenticated `net.Pipe` coverage verifies ranged packet order and final HP. Focused normal/race gates, full ordinary `go test ./...`, `go vet ./...`, and `go build ./...` pass. Full race still reports only the pre-existing GuildBuff equipment-stat/shared-session-fixture race and Blink buff/map-transition race, with no stack entering AI=75.

### 2026-08-26 — Revelation refresh must preserve TucsonEgg hit admission

- Symptom: terminal review found eight monster attack adapters appended the new shared `ObjectHealth` refresh after `tucsonEggAttackedLocked` even when its returned `hit` was false; a follow-up regression test initially failed compile by using `slices.Contains` without first checking the file's imports or the package's existing packet-ID helper.
- Root cause: producer wiring followed the callsite's damage branch but discarded the override's mutation-success result, while the test was written from a remembered standard-library API instead of the verified local declaration.
- Prevention: every refresh producer must carry the actual mutation/hit result into notification admission; TucsonEgg accuracy/armour misses emit no health refresh. Before adding a test helper or package import, locate and reuse the exact in-package declaration. A refresh built from the explicit post-mutation monster value is not stale merely because value-map writeback occurs later in the same locked function before notifications are returned for delivery.
- Verification: Armadillo, EarthGolem, GeneralMeowMeow, Guard, HellLord bomb, OmaWitchDoctor, ScalyBeast and TucsonGeneral now guard refresh on `hit`; `TestTucsonEggMonsterAdaptersDoNotRefreshOnMiss` covers all eight revealed-target adapters and passes count 20 after switching to the verified `containsPacketID` helper. The shared monster routing test already proves packet HP comes from its explicit snapshot rather than a map reread; focused repeated/race and integration gates are rerun after the correction.
