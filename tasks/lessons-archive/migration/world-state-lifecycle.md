...
- Symptom: P11 审查发现 Storage 附件已经在 auth 原子提交，但 world 物品快照直到 `RefreshItem`/结果包写出后才更新；钓鱼 Tick 也先投递通知再持久化，觉醒拆解新增的 `ItemInfos` 没有同步到 world。连接写失败后的 cleanup 可能用旧 world 快照覆盖已提交状态。P6 grid 集成又发现 Sun/Normal Potion 已改 session/world 却未改 auth，随后 Delete 从正确的 auth latest snapshot 提交时把先前药水消耗复活。item-use admission 的首个 runtime-stat 会话测试又直接改派生 `Stats`，随后 no-op latest-auth item apply 重算该字段，生产入口仍按低 DC 拒绝。扩大 focused 集后，no-op refresh 还用 auth 整角色覆盖 session/world 的 intelligent-creature fullness，且旧 contract fixture 的零 requirement mask 被新共同门按 Legacy 正确拒绝。首轮包测试随后暴露 SealedHero 夹具同样省略 masks，公会 class-denial 测试也仍期待旧的 silent failure。最新 auth 原子消费接入后，智能生物 Shape 24 测试又只把 world fullness 改成零，却未推进 world revision 或同步 auth，回调按真实 latest authority 读到 300 并正确拒绝。最终复审进一步发现智能生物领养先改 world、忽略 auth item 写入失败，再持久整快照；Hero 特殊物品和 SealedHero attach 也仍以 world-first 或 stale full snapshot 两阶段覆盖 latest authority。
- Root cause: 把成功响应顺序当成了整个事务顺序，没有区分“auth/world 双权威状态提交”和“可能失败的网络通知”；同时只同步物品格，遗漏了新物品定义目录或把 session-local delta 留给 logout。P6 shout arming 又尝试在 `world.mu` 内调用只返回 bool 的 auth callback，既新增 world→auth 嵌套锁序，又无法取得 rental normalization 后的真实 auth grids。新测试修改了可重建输出而非 `GuildBuffStats` 等输入 authority，错误假设一次加锁写入能跨后续同步保留；新入口又把“刷新 auth 物品格”误实现为整角色 authority 替换，旧夹具则省略了 Legacy 定义必需的 None masks，历史断言也未随新增的可观察本地化包更新。
- Prevention: 所有跨 auth/world 的物品事务先完成 auth 提交、world `ItemInfos`/三类物品格同步及必要落盘，再按 Legacy 顺序投递网络包；禁止让未提交的 session-local 物品差异跨入下一事务。网络失败只能影响通知，不能让 cleanup 从旧快照回滚事务。多个定时结果先选取最后一个 changed 快照持久化，再保持原结果顺序投递全部通知。禁止用持锁 bool callback 伪造原子性；共享事务层必须显式规定 revision/CAS、normalized snapshot 返回值和唯一锁序，由 owning leaf 统一实现。任何 item-use family 即使已由旧 feature leaf 完成，也必须在共享 latest-auth refresh 接入时重新审计 lookup→effect→consume 的单事务边界；world 只能发布已提交 detached snapshot 或显式回滚，Hero registry 与 player/Hero grids 不能通过第二次 stale full commit 收尾。跨同步边界的测试夹具必须写入真实输入 authority 并调用生产重算，禁止只篡改派生快照；直接改 world 状态的夹具还必须推进对应 revision 并同步 auth，才能代表生产可达状态。局部 item refresh 只合并 normalized grids，不得覆盖其他 runtime authority，ItemInfo fixture 必须显式设置真实 requirement masks。
- Verification: `advanceFishing` 已改为先持久化最后一个变化快照再投递 Tick 通知；`EquipSlotItem` 在任何响应写入前同步 world；觉醒持久化同时同步 `ItemInfos` 与物品格，并新增 world/auth 定义一致性测试和网络材料不足事务测试。P6 复审拦截并完整撤回了嵌套 callback；grid leaf 的 basic-potion 路径现先同步 rental-normalized auth/world/disk，再发 health/UseItem，定向 Use→Delete 与 revision/CAS 会话测试通过。runtime-stat 测试改为安装 `GuildBuffStats` 后调用生产重算，原失败命令已重跑 exit 0，并证明 latest-auth apply 后仍按 200 MaxDC 成功消费；no-op refresh 改为仅合并三类 item grids，contract/SealedHero fixtures 补齐 31/3 masks，公会测试改验 System text → failure；Shape 24 夹具同步 world revision/auth fullness 后原测试 `-count=20` 重跑 exit 0。最终 reviewer `01a0398c-2192-7710-baba-e3c5548d5d1a` 又拦截 sealed-Hero 的旧 snapshot attach + 二次 full commit 与 auth 已提交后 ordinary-pet rollback；生产现改为单次 revisioned `AttachSealedHeroLatest`，并只在 item callback 未提交时撤销 pet。对应生产路径 `-count=20`/race `-count=5`、fresh full test/vet/build/full-race 均 exit 0，复审返回 no findings。
