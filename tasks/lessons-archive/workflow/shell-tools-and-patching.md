...
- Recurrence during the same leaf's evaluator repair: a patch intended for the
  `CHECK` flag value matched the earlier identical `CHECKGOLD` ParseUint line.
  The focused whitespace regression failed, the physical range exposed the
  wrong landing, and every numeric TryParse-compatible branch was then patched
  with its unique `case` anchor before the exact test passed.
...
- Strengthening during Start/Logout compaction handoff: durable handoff 初稿把控制脚本要求的固定标题 `## Active leaf and protected work` 改写成同义标题，导致 `tasks/check-migration-control.sh` exit 1；该复合调用全部输出作废。Script Load 收尾又把同一标题自拟为 `## Active leaf and next authority`，再次被检查器拒绝。以后 replace-in-place handoff 必须从现有结构复制固定标题，只替换正文；回读后以控制脚本零退出重跑验证。本次已恢复精确标题并重新执行完整单仓门禁。
