...
- Verification: Equip tracing 又在有效 Server 查询后追加未枚举的 `Shared/Packets` 路径；整次结果作废并只对已确认的两个 Server 文件重跑。缺失 `rg`、猜测文件、错误 glob/schema/commit/path 和被管道掩盖的非零退出均按同一最小验证规则处理；只有权威命令回读的真实路径、schema、完整哈希和零退出结果可作证据。
...
- Strengthening during Refine/Cross-Grid revision integration: non-unique patches landed first in the generic item helper and later put `heroChanged` in `updatePlayerHeroes` instead of `applyCommittedCrossGridItems`. Both were physically reread and corrected with function anchors before compile; even a one-line replacement requires its owning declaration anchor and readback.
...
- Prevention: 优先复用现有 helper；按 seed→load→enter→summon 顺序建立状态；账号 ID 必须匹配 `[A-Za-z0-9]{3,15}`（禁止连字符），并满足真实角色名和配置约束。
