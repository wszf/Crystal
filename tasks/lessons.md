...
- Strengthening through admin/ban-delete and P4: P4 对照已两次在一仓尾部追加对侧文件，整调用作废并分仓重跑。发送前拒绝 `git -C` 与绝对/相对对侧路径，只采用单仓零退出完整输出；改 active lesson 前先 `wc`，本次压缩后 50 KiB 门禁为 0。
...
- Strengthening through rank/P4: `--glob` 越界、可零匹配裸跑及猜 Packet 路径再次导致 exit 2/1；整调用作废并先枚举路径、用固定 argv/显式 0/1 分支重跑。发送前机械核对路径、argv、退出码、输出上限。
