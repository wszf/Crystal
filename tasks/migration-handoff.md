...
| Go migration | `/Users/wszf/Dropbox/source_code/git_work/me_work/Crystal.GoServer` | `main` | `f53167c feat(p5): complete DragonStatue AI` | AI=52/53 为 `e29e1f9`，Dragon session 验证为 `71323ce`，AI=54 为 `f53167c`；工作树干净 |
...
| P5 战斗/技能/怪物/掉落 | In progress | 已完成核心近战、远程、PvP 基础、多个单体/区域魔法、九个自增益 Buff、FrostCrunch 状态、基础属性、掉落树和持久化；最近批次新增玩家 TrapHexagon、SummonVampire/SummonToad/SummonSnakes、CannibalPlant、Guard、Tao Guard、Deer AI=1/2、Tree AI=3、EvilCentipede AI=14、WoomaTaurus AI=11、RedMoonEvil AI=13、Shinsu AI=18、BugBagMaggot AI=12/RootSpider AI=39/BombSpider AI=40、RightGuard/LeftGuard AI=31/32、MinotaurKing AI=33、FrostTiger AI=34、ThunderElement AI=49 GreatFoxSpirit AI=50、Dragon/EvilMir AI=52/53 以及 DragonStatue AI=54 的 Legacy admission、目标捕获、延迟/生命周期、AI/伤害/逃跑、静态/Observer 可见性和 net.Pipe transcript；剩余技能/Buff、飞行/墙体规则、高级 PvP/组队战斗、其他通用/特殊怪物 AI、持久重生状态及完整包序仍待迁移。 |
...
Go 提交 `e29e1f9`/`71323ce` 完成 Dragon、EvilMir 与 EvilMirBody 的生产行为和真实会话验证；提交 `f53167c` 完成 AI=54 `DragonStatue`：

- common population、出生/respawn 方向钳制、永久静止但保留继承 push/blocking；
- Player/owned-Monster/Hero 搜索，Hallucination 不屏蔽 Player；
- 300ms 动作锁、500ms live-target 半径二 MAC 延迟攻击、逐目标 DC/Luck、Shock-after-queue 与睡眠后已排队命中；
- 全部 `Struck` 路径无伤害，但 Legacy SpellObject 的 Slow/Dazed 后续 poison 仍生效；
- Sleeping 仍是合法目标，Player/Hero/Pet 保留动作包，但普通 `Attacked` 返回零；
- Player 与 TurnUndead 致死只进入严格 15 分钟睡眠，不发送 `ObjectDied`，不掉落、不清理、不减少 respawn population；
- 睡眠状态不持久化，restart 后 fresh population 为 awake/full HP；
- 认证双会话 `net.Pipe` 锁定 DragonStatue bootstrap、目标/观察者范围攻击 payload、包序与最终 HP。
...
- `go test ./cmd/crystal-server -run 'DragonStatue|HealingCircle|HornedSorceror|HornedCommander|HellLord|StoningStatue|OmaWitchDoctor|TurnUndead' -count=20 -timeout=300s`
- `go test -race ./cmd/crystal-server -run 'DragonStatue|SessionDragonStatue' -count=5 -timeout=300s`
- `go test ./cmd/crystal-server -count=1 -timeout=600s`（最终重跑通过）
...
无排除项的普通全仓测试两次实际失败于既有 `TestSessionOmaMageRangeSlowFrozenTranscript`，随机边界记录为 `[2 1]`、期望 `[1]`；该测试单独 `-count=20` 亦复现 3 次。完整 `go test -race ./... -count=1 -timeout=1800s` 还复现既有 `TestGuildBuffSessionNewbieLoginReplacesStalePersistedBuff` 数据竞争，栈位于 `player_spell_buffs.go:742,824` 与 `intelligent_creature_items.go:549`。两类失败均未进入 AI=54 代码；未修改无关模块掩盖失败，精确证据已追加到 lessons archive。
